...
In this chapter, we will show how the `=` operator in Elixir is actually a match operator and how to use it to pattern match inside data structures. We will also learn about the pin operator `^` used to access previously bound values.
...
Since there is no variable `unknown` previously defined, Elixir imagines you are trying to call a function named `unknown/0`, but there is no such function.
...
A pattern match will return an error if the sides can't match. This is the case, for example, when the tuples have different sizes:
...
Pattern matching allows developers to easily destructure data types such as tuples and lists. As we will see in the following chapters, it is one of the foundations of recursion in Elixir. It applies to other types, as well, like maps and binaries.
